...
save_figures <- TRUE
output_dir <- "figures_R/diversity"
...
# 定义多样性目标
target_subset <- c("reads", "zotu", "shannon", "simpson")
suffix <- "diversity"
...
if (file.exists("output_group/scatter_data_all.csv")) {
  ...
  print(p1)
  if (save_figures) save_plot(paste0(output_dir, "/Figure1_scatter_", suffix), p1, width = 16, height = 12)
  message("✓ 图1 已生成")
}
...
if (file.exists("output_group/feature_shap_importance_by_model.csv")) {
  ...
  n_col <- min(3, length(targets))  # 4个目标 → n_col=2
  ...
  print(p3)
  if (save_figures) save_plot(paste0(output_dir, "/Figure3_SHAP_heatmap_", suffix), p3, width = 16, height = 8)
  message("✓ 图3 已生成")
}
...
if (file.exists("output_group/feature_shap_importance_by_model.csv")) {
  ...
  # 提前对Group字段换行，确保所有子图使用相同的标签（因子水平一致）
  ...
  for (i in seq_along(targets)) {
    ...
    p <- ggplot(df_target, aes(x = Feature, y = MeanAbsSHAP, fill = Group_wrapped)) +
      ...
      coord_flip() +
      scale_fill_lancet(drop = FALSE, breaks = all_groups) +   # 固定图例顺序和所有组
  ...
  # 组合图形，使用 guides = "collect" 统一收集图例
  p4 <- wrap_plots(shap_plots, ncol = 2, nrow = 3) +
  ...
  print(p4)
  if (save_figures) save_plot(paste0(output_dir, "/Figure4_SHAP_all_features_", suffix), p4, width = 16, height = 14)
  message("✓ 图4 已生成")
}

# ==================== 主图5：SHAP分组贡献比例（图例单行，无副标题） ====================
if (file.exists("output_group/shap_group_contributions_by_model.csv")) {
  ...
  p5 <- ggplot(df_shap_group, aes(x = Group, y = Contribution, fill = Target_label)) +
    geom_bar(stat = "identity", position = position_dodge(0.8), colour = "white", width = 0.7, size = 0.2) +
    scale_fill_jco(name = "Target (Best-performing Model)") +  # 副标题已去除
    ...
    guides(fill = guide_legend(nrow = 1))   # 图例单行
  print(p5)
...
if (file.exists("output_group/oof_with_residuals.csv")) {
  ...
  print(p6)
  if (save_figures) save_plot(paste0(output_dir, "/Figure6_residual_diagnostic_", suffix), p6, width = 12, height = 10)
  message("✓ 图6 已生成")
}

# ==================== 补充图S1：PCA载荷热图（PC1和PC2左右并排，放大字体） ====================
if (file.exists("output_group/pca_loadings_all.csv")) {
  ...
  p_pc2 <- ggplot(df_pc2, aes(x = Target, y = Variable, fill = Loading)) +
    ...
    theme(strip.text.y = element_text(angle = 0, face = "bold", size = 14),
          axis.text.x = element_text(angle = 45, hjust = 1, size = 13),
          axis.text.y = element_blank(),   # 右侧隐藏y轴标签
          legend.position = "right",
          legend.title = element_text(size = 14, face = "bold"),
          legend.text = element_text(size = 12),
          plot.title = element_text(face = "bold", size = 18, hjust = -0.1),
...
if (file.exists("output_group/feature_importance_variation.csv")) {
  ...
  print(pS4)
  if (save_figures) save_plot(paste0(output_dir, "/FigureS4_SHAP_CV_heatmap_", suffix), pS4, width = 10, height = 8)
  message("✓ 附图S4 已生成")
}

# ==================== 附图S5：SHAP排名稳定性（Spearman） ====================
if (file.exists("output_group/shap_rank_spearman_stability.csv")) {
  ...
  print(pS5)
  if (save_figures) save_plot(paste0(output_dir, "/FigureS5_SHAP_rank_spearman_", suffix), pS5, width = 12, height = 8)
  message("✓ 附图S5 已生成")
}
...
message("\n🎉 多样性指标完整分析完成！所有图片保存在 ", output_dir, "/")