...
np.random.seed(22)
os.makedirs("output", exist_ok=True)  # 统一使用 output 目录
...
# 目标变量：只选3个
target_cols = ["reads", "ZOTUs", "Monothalamida"]
# 目标变量：9个有孔虫相关指标
...
# 所有原始特征列（与之前相同，但需确认深度和 chlorophyll a 等已在其中）
feature_cols = [
...
# 外部交叉验证：普通 KFold，随机打乱
outer_cv = KFold(n_splits=5, shuffle=True, random_state=22)
# 内部交叉验证：用于超参数调优，随机打乱
...
def build_pca(train_df, test_df, fold, target, all_loadings, all_variances):
    ...
    for gname, vars_list in groups_dict.items():
        ...
        pca = PCA(n_components=2)   # 改为2个主成分
        ...
        # 记录每个变量的载荷（区分PC1和PC2）
        for comp_idx, comp_name in enumerate(["PC1", "PC2"]):
...
for target in target_names:
    ...
    for fold, (train_idx, val_idx) in enumerate(
            outer_cv.split(df, y)):   # 注意：非空间CV，无需groups
...
# =========================================================
# 5. 保存合并后的输出文件（统一使用 output 目录）
# =========================================================

pd.DataFrame(all_perf).to_csv("output/model_performance_summary.csv", index=False)
pd.concat(all_pred).to_csv("output/scatter_data_all.csv", index=False)
pd.DataFrame(all_shap).to_csv("output/shap_all_models.csv", index=False)
pd.DataFrame(all_best_params).to_csv("output/best_hyperparameters_all_folds.csv", index=False)
pd.DataFrame(gam_lam_records).to_csv("output/gam_lam_records.csv", index=False)

# 保存合并的PCA载荷和方差解释率
pd.DataFrame(all_pca_loadings).to_csv("output/pca_loadings_all.csv", index=False)
pd.DataFrame(all_pca_variances).to_csv("output/pca_variances_all.csv", index=False)

# =========================================================
# 6. 全局SHAP重要性（按目标和模型分组）
# =========================================================

df_shap = pd.read_csv("output/shap_all_models.csv")
df_global_shap = df_shap.groupby(["Target", "Model", "Feature"])["MeanAbsSHAP"].mean().reset_index()
df_global_shap.to_csv("output/feature_shap_importance_by_model.csv", index=False)
...
df_group_shap.to_csv("output/shap_group_contributions_by_model.csv", index=False)
...
vif_data.to_csv("output/vif_original_features.csv", index=False)
...
vif_pc_data.to_csv("output/vif_pc_features.csv", index=False)

# =========================================================
# 8. 自动生成论文表格（英文命名）
# =========================================================

# 表1：各区域样本数量分布（非空间CV中仍保留区域统计）
df["Area"].value_counts().reset_index().rename(
    columns={"index": "Area", "Area": "Count"}
).to_csv("output/Table1_Area_Distribution.csv", index=False)

# 表2：各目标变量下各模型的平均性能（跨fold取均值）
perf_mean = pd.DataFrame(all_perf).groupby(["Target", "Model"]).mean().reset_index()
perf_mean.to_csv("output/Table2_Model_Performance_Summary.csv", index=False)

# 表3：每个目标变量的最佳模型（基于平均R2最高）
best_model = perf_mean.loc[perf_mean.groupby("Target")["R2"].idxmax()]
best_model.to_csv("output/Table3_Best_Model_per_Target.csv", index=False)
...
top5_shap.to_csv("output/Table4_SHAP_Top5_Features_XGBoost.csv", index=False)

# =========================================================
# 9. SHAP稳定性分析
# =========================================================

df_shap_full = pd.read_csv("output/shap_all_models.csv")
...
shap_stability.to_csv("output/shap_stability_mean_sd_cv.csv", index=False)
...
freq_table.to_csv("output/shap_top_frequency.csv", index=False)
...
df_params = pd.read_csv("output/best_hyperparameters_all_folds.csv")

param_summary = df_params.groupby(["Model","Target"]).agg(
    ["mean","std"]
)

param_summary.to_csv("output/hyperparameter_stability_summary.csv")
...
df_loadings = pd.read_csv("output/pca_loadings_all.csv")

pca_stability = df_loadings.groupby(
    ["Target", "Group", "PC", "Variable"]   # 增加了 "PC"
)["Loading"].agg(["mean", "std"]).reset_index()

pca_stability["CV"] = abs(pca_stability["std"] / pca_stability["mean"])

pca_stability.to_csv("output/pca_loading_stability.csv", index=False)
...
df_gam = pd.read_csv("output/gam_lam_records.csv")
...
gam_stability.to_csv("output/gam_stability_summary.csv")
...
df_perf = pd.read_csv("output/model_performance_summary.csv")
df_pred = pd.read_csv("output/scatter_data_all.csv")
...
pd.concat(oof_records).to_csv(
    "output/oof_predictions_best_models.csv",
    index=False
)
...
oof_df = pd.read_csv("output/oof_predictions_best_models.csv")
oof_df["Residual"] = oof_df["Observed"] - oof_df["Predicted"]

oof_df.to_csv("output/oof_with_residuals.csv", index=False)
...
importance_cv.to_csv(
    "output/feature_importance_variation.csv",
    index=False
)
...
df_rank_stability.to_csv("output/shap_rank_spearman_stability.csv", index=False)
...
df_perf_full = pd.read_csv("output/model_performance_summary.csv")
...
pd.DataFrame(gap_records).to_csv(
    "output/train_test_gap_analysis.csv",
    index=False
)
...
cv_summary.to_csv(
    "output/model_performance_cv_summary.csv",
    index=False
)
...
df_best = pd.read_csv("output/Table3_Best_Model_per_Target.csv")
df_pred = pd.read_csv("output/scatter_data_all.csv")
...
best_scatter.to_csv(
    "output/best_model_scatter_data.csv",
    index=False
)
...
df_res = pd.read_csv("output/oof_with_residuals.csv")
...
residual_summary.to_csv(
    "output/residual_distribution_summary.csv",
    index=False
)
