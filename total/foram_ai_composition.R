...
save_figures <- TRUE
output_dir <- "figures_R/composition"
...
# 定义组成目标（5个）
target_subset <- c("Rotaliida", "Textulariida", "Monothalamida", "Miliolida", "Other")
suffix <- "composition"
...
if (file.exists("output_group/scatter_data_all.csv")) {
  ...
  print(p1)
  if (save_figures) save_plot(paste0(output_dir, "/Figure1_scatter_", suffix), p1, width = 16, height = 14)
  message("✓ 图1 已生成")
}
...
if (file.exists("output_group/feature_shap_importance_by_model.csv")) {
  ...
  n_col <- min(3, length(targets))  # 5个目标 → n_col=3
  ...
  print(p3)
  if (save_figures) save_plot(paste0(output_dir, "/Figure3_SHAP_heatmap_", suffix), p3, width = 18, height = 8)
  message("✓ 图3 已生成")
}
...
if (file.exists("output_group/feature_shap_importance_by_model.csv")) {
  ...
  # 5个目标，排列为3列2行（3+2）
  p4 <- wrap_plots(shap_plots, ncol = 3, nrow = 2) +
  ...
  print(p4)
  if (save_figures) save_plot(paste0(output_dir, "/Figure4_SHAP_all_features_", suffix), p4, width = 18, height = 12)
  message("✓ 图4 已生成")
}
